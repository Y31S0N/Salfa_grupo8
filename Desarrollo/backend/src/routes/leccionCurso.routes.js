...
router.post('/modulos/:moduloId/lecciones', async (req, res) => {
    ...
    try {
        ...
        if (!modulo) {
            return res.status(404).json({ error: "No se encontró ningún módulo con ese ID" });
        }
...
router.get('/modulos/:moduloId/lecciones', async (req, res) => {
    const moduloId = parseInt(req.params.moduloId);
...
router.get('/modulos/:moduloId/lecciones/:leccionId', async (req, res) => {
    ...
    try {
        ...
        if (!leccion || leccion.moduloId !== parseInt(moduloId)) {
            return res.status(404).json({ error: "No se encontró ninguna lección con ese ID para el módulo especificado" });
        }

        return res.json(leccion);
    } catch (error) {
        console.error("Error al obtener la lección:", error.message);
        return res.status(500).json({ error: "Error al obtener la lección", details: error.message });
    } finally {
...
router.put('/modulos/:moduloId/lecciones/:leccionId', async (req, res) => {
    const { moduloId, leccionId } = req.params;
    const { nombre_leccion, descripcion_leccion } = req.body;
    ...
    try {
        ...
        if (isNaN(leccionIdNumber) || isNaN(moduloIdNumber)) {
            return res.status(400).json({ error: "El ID de la lección y el ID del módulo deben ser números válidos" });
        }
        ...
        if (descripcion_leccion) updatedData.descripcion_leccion = descripcion_leccion;
        ...
        res.status(500).json({ error: "Error al modificar la lección", details: error.message });
    }
});


// Endpoint para eliminar una lección específica de un módulo
router.delete('/modulos/:moduloId/lecciones/:leccionId', async (req, res) => {
    const { moduloId, leccionId } = req.params;
