...
// Endpoint para crear un módulo para un curso existente
...
router.get('/cursos/:cursoId/modulos/:moduloId', async (req, res) => {
    ...
    try {
        ...
        if (!modulo || modulo.cursoId !== parseInt(cursoId)) {
            return res.status(404).json({ error: "No hay ningún módulo con ese ID para el curso especificado" });
        }
...
router.put('/cursos/:cursoId/modulos/:moduloId', async (req, res) => {
    const { cursoId, moduloId } = req.params; // Obtenemos los IDs del curso y del módulo desde la URL
    ...
    try {
        ...
        const updatedModulo = await prisma.modulo.update({
            where: { id_modulo: parseInt(moduloId) },
            data: {
                nombre_modulo: nombre_modulo !== undefined ? nombre_modulo : modulo.nombre_modulo,
                descripcion_modulo: descripcion_modulo !== undefined ? descripcion_modulo : modulo.descripcion_modulo,
...
router.delete('/cursos/:cursoId/modulos/:moduloId', async (req, res) => {
    const { cursoId, moduloId } = req.params; // Obtenemos los IDs del curso y del módulo desde la URL
