...
const Agregar_modulos = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:5000/api/cursos/${id}/modulos`, nuevoModulo);
  ...
  return (
    <div className="flex justify-center items-center min-h-screen">
      <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
        <h2 className="text-2xl font-bold mb-4 text-center">Agregar Nuevo Módulo</h2>
        
        {/* Mostrar el nombre del curso */}
        <p className="text-lg mb-4 text-center text-gray-700">
          Creando módulo para el curso: <span className="font-semibold">{nombreCurso}</span>
        </p>
