...
export default function DetalleUsuarioLecciones() {
  ...
  const generatePDF = () => {
    ...
    doc.setFontSize(12);
    doc.text(`Usuario: ${usuario.nombre} ${usuario.apellido_paterno} ${usuario.apellido_materno}`, 14, yPos);
    yPos += 10;
    ...
    usuario.cursoAsignados.forEach((cursoAsignado, index) => {
      ...
      const estado = progreso === 100 ? "Completado" : 
      ...
      doc.text(`Fecha de Asignación: ${new Date(cursoAsignado.fecha_asignacion).toLocaleDateString()}`, 20, yPos);
      yPos += 5;
      ...
      const leccionesData = curso.modulos.flatMap(modulo => 
        modulo.lecciones.map(leccion => {
          const completada = leccion.Cumplimiento_leccion && 
            leccion.Cumplimiento_leccion.length > 0 && 
            leccion.Cumplimiento_leccion[0].estado;
    ...
    // Guardar el PDF
    doc.save(`informe_cursos_${usuario.nombre}_${usuario.apellido_paterno}.pdf`);
  };
