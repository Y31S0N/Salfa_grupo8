...
// Define la interfaz para la lección
interface Leccion {
  id_leccion: number;
  nombre_leccion: string;
  descripcion_leccion: string;
  fecha_de_creacion_leccion: string;
}

const Modificar_lecciones = () => {
  const { cursoId, moduloId, leccionId } = useParams<{ cursoId: string; moduloId: string; leccionId: string }>();
  console.log(cursoId);
  ...
  useEffect(() => {
    const obtenerDatos = async () => {
      try {
        const response = await axios.get<Leccion>(`http://localhost:5000/api/modulos/${moduloId}/lecciones/${leccionId}`);
        setLeccion(response.data);
      } catch (error) {
        setError(error instanceof Error ? error.message : "Error al obtener la lección.");
        console.error(error);
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/modulos/${moduloId}/lecciones/${id_leccion}`, {
        nombre_leccion,
        descripcion_leccion,
      });
      ...
      setError(error instanceof Error ? error.message : "Error al modificar la lección.");
      console.error(error);
    }
  };
  ...
  return (
    <div className="flex justify-center items-center min-h-screen">
      <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
        <h2 className="text-2xl font-bold mb-4 text-center">Modificar Lección</h2>
