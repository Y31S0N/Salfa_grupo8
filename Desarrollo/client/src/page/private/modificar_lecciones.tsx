...
const Modificar_lecciones = () => {
  ...
  useEffect(() => {
    const obtenerDatos = async () => {
      try {
        const response = await axios.get<Leccion>(
          `http://localhost:3000/modulos/${moduloId}/lecciones/${leccionId}`
        );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await axios.put(
        `http://localhost:3000/modulos/${moduloId}/lecciones/${id_leccion}`,
        {
          nombre_leccion,
          descripcion_leccion,
        }
